...
function deriveKey(secret: string, householdId: string): Buffer {
  return scryptSync(`${secret}:${householdId}`, 'time-is-money-aes', 32);
}
...
export function encryptSensitiveField(
  ...
  const key = deriveKey(secret, householdId);
